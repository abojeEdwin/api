    ...
    class Meta:
        ...
        def clean_id_no(self):
            ...
            if User.objects.filter(id_no=id_no).exists():
                raise ValidationError(_("A user with that ID already exist."))
            return id_no
            ...
            if not is_superuser:
                if not security_question:
                    self.add_error("security_question",
                               _("Security question is required."),)
...
class UserChangeForm(DjangoUserChangeForm):
    class Meta:
        ...
        def clean_id_no(self):
            id_no = self.clean_data.get('id_no')
            if User.objects.exclude(pk=id_no).filter(id_no=id_no).exists():
                raise ValidationError(_("A user with that ID already exist."))
            return id_no
