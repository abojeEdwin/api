...
def send_otp_email(email, otp):
    ...
    try:
        ...
        logger.error(f"Failed to send OTP email to: {email} with error: {str(e)}")
...
def send_account_locked_email(self):
    ...
    context = {
        "user": self,
        "lockout_duration": int(settings.LOCKED_DURATION.total_seconds() // 60),
        "site_name": settings.SITE_NAME,
    }
    ...
    try:
        email.send()
        logger.info(f"Account locked emil sent to : {self.email}")
    except Exception as e:
        logger.error(f"Failed to send OTP email to: {self.email} with error: {str(e)}")
